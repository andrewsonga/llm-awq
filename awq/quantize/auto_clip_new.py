    ...
    for i_b in range(w.shape[0] // oc_batch_size):
        ...
        for i_s in range(int(max_shrink * n_grid)):
            ...
            if q_config["act_quant"] == "none":
                input_feat_quant = input_feat.clone()
            elif q_config["act_quant"] == "per_tensor":
                input_feat_quant = quantize_activation_per_tensor_absmax(
                    input_feat.clone(), n_bits=q_config["a_n_bits"]
                )
            elif q_config["act_quant"] == "per_token":
                input_feat_quant = quantize_activation_per_token_absmax(
                    input_feat.clone(), n_bits=q_config["a_n_bits"]
                )
            elif q_config["act_quant"] == "per_channel":
                input_feat_quant = quantize_activation_per_channel_absmax(
                    input_feat.clone(), n_bits=q_config["a_n_bits"]
                )
            else:
                raise NotImplementedError

            cur_out = (input_feat_quant * q_w).sum(dim=-1)

            # co, 1, n_group, 1
            err = (cur_out - org_out).pow(2).mean(dim=1).view(min_errs.shape)
            # print(f"i_s: {i_s}, error: {err}")
